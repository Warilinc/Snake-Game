...
public:
    ...
    bool once_more();           // вывод запроса и приЄм ответа от игрока
...
private:
    ...
    enum class State { ST_OK, ST_EXIT, ST_DIED };  //состо€ни€ игры

    typedef std::pair<int, Command> CmdPair;

    int width, height;          //размеры пол€
    int latency;                //задержка между действи€ми
    ...
    WORD bdtxtColor = 7;                //цвет границ игрового пол€ и текста
